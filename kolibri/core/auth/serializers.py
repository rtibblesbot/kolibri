...
class RoleListSerializer(serializers.ListSerializer):
    def validate(self, attrs):
        for item in attrs:
            ...
            if collection and collection.kind in (
            ...
            if (
                collection
                and collection.kind == collection_kinds.CLASSROOM
                and item.get("kind") != role_kinds.COACH
            ):
                raise serializers.ValidationError(
                    "Can only assign Coach roles to Classrooms"
                )
        return attrs
