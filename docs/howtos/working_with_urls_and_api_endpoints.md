...
//          @action(detail=False, methods=['post']) def import_from(self, request)
// Each registers a URL named kolibri:core:contentnode_<action>, which `action` selects.
...
export default new Resource({
  ...
  async importFrom(source) {
    const { data } = await this.request({ method: 'POST', action: 'import_from', data: source });
    return data;
  },
});
```
