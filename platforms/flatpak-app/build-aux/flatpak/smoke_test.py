...
def phase_update():
    ...
    def restarted_on_new_commit():
        starts = _kolibri_start_count()
        if (
            _gdbus_call("", "", "org.gtk.Actions.Activate", "restart", "[]", "{}")
            is None
        ):
            return False
        return _wait_for(
            lambda: (
                _kolibri_start_count() > starts
                and all(
                    [_app_commit(pid) for pid in pids()] == [new_commit]
                    for pids in (_frontend_pids, _daemon_pids)
                )
            ),
            PHASE_WAIT_S,
        )
    ...
    checks = {
        ...
        "restarted-on-new-commit": restarted_on_new_commit(),
    }
    print(
        "  " + " ".join(f"{name}={passed}" for name, passed in checks.items()),
        flush=True,
